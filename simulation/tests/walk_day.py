...
async def main() -> int:
    async with websockets.connect(URL, max_size=8 * 1024 * 1024) as ws:
        ...
        # Built from the snapshot, like the cells below it. A hand-written header
        # silently stops matching its own columns the moment a device is added.
        head = " ".join(f"{d['label'].split()[0]:>11}" for d in snap["devices"])
        header = f"  hour  ambient {head}   source"
        print(header)
        print("  " + "-" * (len(header) - 2))
