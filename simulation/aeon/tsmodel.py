...
# Observed history shorter than this is ignored rather than trained on: it is
# not enough to build a window from, and half a day of real data is a worse
# teacher than a coherent synthetic one.
MIN_OBSERVED_STEPS = sequence.WINDOW + 8
...
class TrainResult:
    ...
    # How much of the training set came from observed behaviour rather than from
    # restated rules. Worth reporting: it is the difference between "the model
    # learned what I told it" and "the model learned what the house does".
    stated_windows: int = 0
    observed_windows: int = 0
    observed_hours: dict[str, int] = field(default_factory=dict)
    # The deployed model's AUC re-measured on THIS dataset, not the figure it was
    # given when it was trained. None when it could not be scored.
    incumbent_auc_measured: float | None = None
...
# ── building a timeline from what actually happened ──────────────────────

def timeline_from_usage(device_id: str, usage_rows,
                        min_steps: int = MIN_OBSERVED_STEPS) -> list[Step]:
    """Collected usage -> one Step per hour.

    `synthesise()` above answers "what did you say you wanted?". This answers
    "what did the house actually do?", which is the only one of the two that can
    disagree with the stated rule -- an override at 23:00, a leaf that was
    offline, an empty room on a Tuesday.

    The usage table is event-driven: a row is written when a device changes, so
    a quiet hour has no row at all and a busy one has several. The sequence model
    needs exactly one step per hour, contiguous and in order. So bucket rows into
    absolute hour slots, keep the last state recorded in each, and carry it
    forward across silent hours -- silence is what "nothing changed" looks like.

    Returns [] unless there is enough contiguous history to build real windows.
    A short observed history is worse than none: it trains the model on a day
    that never finished happening.
    """
    rows = [r for r in usage_rows if r["device"] == device_id]
    if not rows:
        return []

    buckets: dict[int, dict] = {}
    for row in sorted(rows, key=lambda r: r["ts"]):
        buckets[int(row["ts"] // sequence.STEP_SECONDS)] = row   # last write in the hour wins

    first, last = min(buckets), max(buckets)
    if last - first + 1 < min_steps:
        return []

    timeline: list[Step] = []
    carried: dict | None = None
    for slot in range(first, last + 1):
        carried = buckets.get(slot, carried)
        if carried is None:
            continue
        on = bool(carried["on_state"])
        ts = slot * sequence.STEP_SECONDS
        timeline.append(Step(
            on=on,
            # Off steps carry no level here for the same reason they carry none
            # anywhere else: writing the last known level into an off step
            # poisons the lag window, and does it silently.
            level=carried["level"] if on else None,
            occupied=bool(carried["occupied"]),
            # Usage rows record what was switched, not how warm the room was.
            # Ambient comes from the shared curve so that a window built from
            # observation and one built from a stated rule remain comparable.
            ambient_c=ambient_for_hour(time.localtime(ts).tm_hour),
            ts=ts,
        ))
    return timeline
...
def train(active_rows, incumbent_auc: float | None = None,
          seed: int = 0, usage_rows=None, incumbent_onnx: bytes | None = None) -> TrainResult:
    """One training run, all devices pooled into one model.

    A single model serves all four appliances; each window carries a device
    ...
    Two sources are pooled, not swapped:

    * the timeline synthesised from what you *said* (`synthesise`), and
    * the timeline of what the house actually *did* (`timeline_from_usage`).

    Pooled rather than swapped on purpose. Observed history starts at zero hours
    and grows one row at a time, so a run that trained on it alone would fall
    under MIN_WINDOWS and be rejected outright -- Retrain would simply stop
    working on a fresh install, which is exactly when someone is watching. This
    way the stated rules carry the model on day one and real behaviour takes
    over as it accumulates, without a cliff in between.
    """
    t0 = time.perf_counter()
    ...
    stated_windows = observed_windows = 0

    for device_id in devices.DEVICE_ORDER:
        stated = synthesise(device_id, active_rows)
        # Warm start ships the stated day: it is complete, hour-indexed and
        # always 24 steps long, which observed history is not guaranteed to be.
        timelines[device_id] = stated

        sources = [("stated", stated)]
        observed = timeline_from_usage(device_id, usage_rows or [])
        if observed:
            sources.append(("observed", observed))
            observed_counts[device_id] = len(observed)

        for label, timeline in sources:
            X, y_on, y_level = sequence.build_windows(device_id, timeline)
            if not len(X):
                continue
            if label == "observed":
                observed_windows += len(X)
            else:
                stated_windows += len(X)
            Xs.append(X)
            y_ons.append(y_on)
    ...
    # Guardrail 3: a candidate must beat the incumbent -- on THIS dataset.
    # Re-score the deployed model on the same windows where we can; its stored
    # figure describes a different, easier problem. See score_deployed().
    measured = score_deployed(incumbent_onnx, X, y_on) if incumbent_onnx else None
    baseline = measured if measured is not None else incumbent_auc

    if baseline is not None and cv_auc is not None and cv_auc < baseline - 1e-9:
        how = "re-scored on this data" if measured is not None else "as last recorded"
        return TrainResult(ok=False, n_windows=len(X), cv_auc=cv_auc,
                           stated_windows=stated_windows,
                           observed_windows=observed_windows,
                           observed_hours=observed_counts,
                           incumbent_auc_measured=measured,
...
def score_deployed(onnx_bytes: bytes, X: np.ndarray, y: np.ndarray) -> float | None:
    """AUC of the ALREADY-DEPLOYED model, measured on the CURRENT dataset.

    This exists because comparing a candidate's fresh score against the
    incumbent's stored one is not a comparison. The incumbent's number was
    measured on the data that existed when it was trained; every hour the house
    records changes the dataset, and a model trained on richer, messier, more
    realistic behaviour scores LOWER than one trained on a clean synthetic
    timeline while being unambiguously better.

    Judged that way, the first genuinely-informed model is always rejected in
    favour of the naive one that came before it, and the system quietly stops
    being able to learn.

    So the incumbent gets re-scored here, on exactly the windows the candidate
    was judged on. It never saw them either, so the comparison is fair in both
    directions. Returns None if it cannot be scored, and the caller then falls
    back to the stored figure rather than blocking the deploy.
    """
    if not onnx_bytes or len(X) == 0:
        return None
    try:
        ...
        if len(np.unique(y)) < 2:
            return None
        runner = NodeRunner(onnx_bytes)
        scores = np.fromiter(
            (runner.run(X[i:i + 1])[0] for i in range(len(X))),
