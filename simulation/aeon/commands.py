...
_DEVICE_WORDS: list[tuple[str, str]] = [
    ...
    # Last: these are the broadest words in the table, and _find_device returns
    # the first pattern that matches. "clean the light" should still be the light.
    (r"\b(vacuum|hoover|roomba|robot|cleaner|clean\w*|jhaadu|jhadu|safai)\b",
     "vacuum.home"),
]
...
def _find_level(low: str, device_id: str | None, on: bool) -> float | None:
    ...
    if device_id == "fan.bedroom" and re.search(r"\bfull\b|\bmax\w*\b|\bfast\b|\btez\b", low):
        return d.hi
    if device_id == "vacuum.home" and re.search(
            r"\bfull\b|\bmax\w*\b|\bdeep\b|\bturbo\b|\bthorough\w*\b", low):
        return d.hi
...
def resolve(intent: Intent, last: dict | None,
            now_hour: int | None = None) -> Intent | None:
    ...
    `now_hour` is what the clock says when the sentence was spoken. Say "set the
    AC to 23" with no time and you mean *now*, not *always*: this pins it to the
    current hour. Without it the intent falls back to 0-24, which reads as
    reasonable and is not -- it silently turns one instruction into a standing
    round-the-clock rule, and the model then learns to run the AC at 4 AM.

    The clock is passed in rather than read here so this stays pure and the
    demo clock and the wall clock cannot disagree about what "now" means.
    """
    if not intent.is_followup and intent.device is not None:
        if intent.hour_start is None:
            if now_hour is None:
                intent.hour_start, intent.hour_end = 0, 24
            else:
                hour = int(now_hour) % 24
                intent.hour_start, intent.hour_end = hour, hour + 1
        return intent
