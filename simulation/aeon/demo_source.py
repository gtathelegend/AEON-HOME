...
class ScriptedPolicy:
    ...
    follows ambient temperature, the light follows time of day, and the vacuum
    follows the hour it was told to clean.
    """
    ...
    def __init__(self) -> None:
        ...
        self.preferences: dict[str, list[tuple[int, int, bool, float | None]]] = {
            ...
            # Mirrors the seed in PCHub.seed_defaults(). Phase 1 is the fallback
            # that gets demoed when something else is broken, so a device that
            # only works in Phase 2 is a device that fails exactly when it matters.
            "vacuum.home": [(10, 12, True, 70.0)],
        }
    ...
    def state_for(self, device_id: str, hour: int, temp_c: float) -> tuple[bool, float | None, float]:
        ...
        if device_id == "vacuum.home":
            # Outside its window it is confidently idle, not unknown: falling
            # through to the 0.0 default would gate to "abstain" and leave the
            # tile reading CONFIDENCE 0.00 all day.
...
class ScriptedHouse:
    ...
    def _refresh_learned_week(self) -> None:
        ...
        rows.append({"device": "*", "label": devices.occupancy_rule_label(),
                     "text": "Off when the room is empty"})
