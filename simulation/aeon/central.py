...
class CentralNode:
    def __init__(self, ckpt_path: str | Path, spool_path: str | Path,
        ...
        # Mirrors ckpt.automation so the hot path is one attribute read, not a
        # checkpoint field lookup on every device on every tick.
        self.automation: bool = True
        self.schedule: dict = {}
    ...
    async def restore(self) -> bool:
        ...
        self.ckpt = ckpt
        # Restore the consent switch before anything can tick. A node that came
        # back up automating a house whose owner had switched automation off
        # would be doing the one thing they explicitly forbade.
        self.automation = bool(getattr(ckpt, "automation", True))
    ...
    async def set_automation(self, enabled: bool) -> bool:
        """Grant or withdraw the model's authority to act.

        Checkpointed immediately rather than at the next interval save. The gap
        between "the owner switched it off" and "the node wrote that down" is
        precisely the window in which a power cut brings the house back up
        automating against their wishes.
        """
        if self.automation == bool(enabled):
            return self.automation
        self.automation = bool(enabled)
        await self.save("automation")
        return self.automation
    ...
    async def save(self, reason: str = "interval") -> float:
        ...
        self.ckpt.automation = self.automation
    ...
    def _emit_usage(self, device_id: str, on: bool, level: float | None,
        ...
        """
        # Stamp the clock the node is REASONING in, not the wall clock. These
        # rows are training data: the model reads hour-of-day out of this
        # timestamp, and inference, the warm start and the lag window all run on
        # last_tick_ts. Using time.time() here recorded the right appliance state
        # against the wrong hour -- every row of a simulated day landed in
        # whatever real minute it happened to be, and a model trained on that
        # learns a house whose lights come on at 3 AM.
        token = protocol.usage(device=device_id, on=on, level=level,
                               occupied=self.occupied, src=src,
    ...
    async def tick(self, hour: int, is_weekend: bool, ambient: float,
        ...
        for device_id in devices.DEVICE_ORDER:
            ...
            # The consent switch. With automation off the node still infers, still
            # records, still learns -- it simply does not touch the appliance. So
            # the lag window stays continuous and the confidence keeps updating,
            # and switching automation back on resumes from a warm model rather
            # than from a blind one.
            #
            # The occupancy rule is suppressed too. It is automation as much as
            # the model is, and "off means off" is worth more here than a partial
