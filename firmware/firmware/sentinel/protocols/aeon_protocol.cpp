...
void AeonProtocol::sendDreamCompleted(uint32_t duration_ms, uint16_t consolidated_memories) {
...
void AeonProtocol::sendLearningSummary(float overall_score, uint16_t feedback_count) {
    StaticJsonDocument<192> doc;
...
void AeonProtocol::sendFeedbackReceived(const char* feedback_type, const char* target, float value) {
    StaticJsonDocument<256> doc;
...
void AeonProtocol::sendRecommendationGenerated(const char* rec_id, const char* description) {
    StaticJsonDocument<256> doc;
...
void AeonProtocol::sendPolicyAdapted(const char* policy_name, float new_weight) {
    StaticJsonDocument<192> doc;
...
void AeonProtocol::sendRuntimeEvaluation(float decision_quality, float override_freq) {
    StaticJsonDocument<192> doc;
