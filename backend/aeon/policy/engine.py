"""
aeon/policy/engine.py — Policy Engine.

Receives FeatureFrames from the serial bridge, runs QNN inference,
and generates actuation commands + capability tokens.

Design principles:
  - Rules are data (stored in the knowledge graph / memory store).
  - AI output augments rules; it never silently overrides them.
