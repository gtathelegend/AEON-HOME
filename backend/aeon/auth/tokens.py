"""
aeon/auth/tokens.py — Capability token system.

ÆON uses a capability-token model instead of raw data sharing:
  - Sensors generate feature vectors (no raw data leaves Arduino)
  - The policy engine decides to act and issues a signed JWT capability token
  - The token encodes: who, what, why, when — but never raw sensor values
  - Dashboard and mobile verify tokens before acting

This preserves privacy: an intercepted token reveals only the capability
(e.g. "motion detected, confidence 0.92") not the underlying sensor stream.
"""
