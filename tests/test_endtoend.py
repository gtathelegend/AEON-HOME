...
async def main() -> int:
    async with websockets.connect(URL, max_size=4 * 1024 * 1024) as ws:
        ...
        check("all three devices present", len(state["devices"]) == 3)
        ...
        state, ok = await until(ws, lambda s: s["policy"]["model_v"] > before_v, timeout=20.0)
        check("retrain deploys the preferences spoken since the last deploy", ok,
              f"{before_v} -> {state['policy']['model_v']} "
              f"(is the hub running with --reset?)")
        check("deploy logged", bool(events(state, "deploy")))

        # And an immediate second retrain must NOT mint a version for identical
        # content, or the deployment log stops answering "when did behaviour
        # actually change?".
        settled_v = state["policy"]["model_v"]
        await ws.send(json.dumps({"typ": "retrain"}))
        state, ok = await until(
            ws, lambda s: any("unchanged" in e.get("text", "")
                              for e in events(s, "train")), timeout=20.0)
        check("retraining an unchanged policy reports no change", ok,
              str([e.get("text") for e in events(state, "train")][:2]))
