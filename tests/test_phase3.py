...
def test_shapes() -> None:
    ...
    check("input dimension is 105",
          sequence.INPUT_DIM == 105, str(sequence.INPUT_DIM))
    check("96 window + 6 context + 3 device one-hot",
          sequence.WINDOW * sequence.CHANNELS == 96
          and sequence.CONTEXT == 6
          and len(devices.DEVICE_ORDER) == 3)
    check("parameter count is 6,850", tsmodel.count_params() == 6850,
    ...
    check("model_input is [1, 105]", x.shape == (1, 105), str(x.shape))
...
def test_synthesis() -> None:
    ...
    check("windows have 105 features", X.shape[1] == 105, str(X.shape))
...
def test_training() -> tsmodel.TrainResult:
    ...
    check("pooled windows across all three devices", result.n_windows > 1500,
